...
/**
  ...
  */
object ProcessingTime extends App {
  ...
  val timetypedEvents = events.selectExpr("cast(createdAt as timestamp) eventTime",
    "current_timestamp() as processingTime", "deviceId", "signal")

  // calculate Number of events by device in 20 second time windows, every 10 seconds
  val windowedCount = timetypedEvents.groupBy(window($"processingTime", "20 seconds", "10 seconds" ),
    $"deviceId")
    .count()
